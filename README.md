...
SVN（Subversion）版本控制面板 for DeepSeek Harness Web GUI。以独立 tab 融入 `dsh-better-sidebar` 右侧边栏，覆盖 IDEA SVN 的全部基本功能，并注入 `svn_*` 模型工具让 Agent 也能直接操作 SVN。
...
| **.gitignore 忽略** | 自动读取工作副本（含父目录）的 `.gitignore`，按 gitignore 规则把匹配的未版本化文件归入「已忽略」分组并默认隐藏（Git 同款） |
...
| **历史记录** | `svn log --xml -v`，Git 风格历史行（r123 + 摘要 / 作者 · 时间 · 文件数）；**选中提交的详情区悬浮固定在底部**，列表独立滚动 |
| **搁置** | `svn shelve / unshelve / shelve-delete / shelve --list`（IDEA 同款） |
...
> **UI 风格**：与 dsh-better-sidebar 的「源代码管理（Git）」面板共用同一套 DSW 设计 token
> （`--dsw-alias-*` 颜色 / `--dsw-font-*` 字号）与图标（`@deepseek-ai/dsh-client-ui-primitives`），
> 视觉上保持一致。

## 前置

- DSH 0.1.0-rc.8+（`dsh web` 可运行）
- `dsh-better-sidebar` ≥ 0.14.0（提供 `ctx.betterSidebar` 服务与侧边栏宿主）
- 系统 `svn` 命令行（Windows 上 TortoiseSVN 自带 `svn.exe`，需在 PATH）
